package com.axonivy.utils.casemailcomponent.model;
...
public abstract class AbstractBusinessDataLazyDataModel<T> extends LazyDataModel<T> {
	...
	public List<T> load(int first, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {
		...
		Result<T> result = filter(ivy.repo.search(businessDataType)).orderBy().field(sort.toField())
				.direction(toDirection(sort.toOrder())).limit(first, pageSize).execute();
